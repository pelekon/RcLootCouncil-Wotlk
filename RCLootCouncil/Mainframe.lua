...
-----------------------------------------------------------------------------------
--- Manualy implemented functions due to differences between MoP and Wotlk APIs ---
-----------------------------------------------------------------------------------
function IsInRaid()
...
function GetNumGroupMembers()
	...
	-- first check number of players in raid
	if raidNum > 0 then
		return raidNum
	end

	-- if there is no raid then just return number of players in party as it returns 0 if there is no party present
	return GetNumPartyMembers()
end

function GetAverageItemLevel()
	-- TODO: Implement some way to calculate iLvL for player as client doesn't support it
	return 0
end
		...
		if isMasterLooter and isRunning and (IsInRaid() or nnp) then -- if we're masterlooter and addon is enabled or debug is on
			...
			if GetNumLootItems() >= 1 then -- if there's something to loot
				if not InCombatLockdown() then
					bossName = GetUnitName("target", false) -- extract the boss name before the player can switch targets
					...
					for i = 1, GetNumLootItems() do -- run through the number of items in the loot window
						...
						if db.autoAward and (LootSlotIsItem(i) == 1) and lootQuantity > 0 and (lootRarity >= db.autoAwardQualityLower and lootRarity <= db.autoAwardQualityUpper) then
							RCLootCouncil:AutoAward(i, db.autoAwardTo, GetLootSlotLink(i))

						elseif db.autoLooting then
							if ((LootSlotIsItem(i) == 1) or db.lootEverything) and lootQuantity > 0 and lootRarity >= GetLootThreshold() then -- Check wether we want to loot the item or not
	...
	elseif event == "RAID_INSTANCE_WELCOME" then
		...
		if IsInRaid() or nnp then -- if the player is in a raid
			...
			if lootMethod == 'master' then -- if master looter is turned on
				...
				masterLooter = RCLootCouncil_Mainframe.getML()
			elseif (IsRealRaidLeader() == 1) and not isRunning then -- otherwise ask the raid leader
...
--]]
	...
	if sender == GetUnitName("player", false) and distri ~= "WHISPER" then 
		return -- don't do anything if we send the message, unless it was a whisper
	end 
	-- ~start
		if cmd == 'start' and (isCouncil or isMasterLooter) then
			...
			if not lootTable then
				self:Send("RAID", "req_loot", UnitName("player"))
				return self:ScheduleTimer("HandleMessage", 5, cmd, data, distri, sender)
			end
		...
		elseif cmd == 'req_loot' then
			if isMasterLooter then
				self:SendTo(data[1], "lootTable", lootTable)
			end
		...
		elseif cmd == 'verTest' then -- someone sent us a version test
			if version < data[1] and not hasVerCheck then -- if we're outdated and haven't already showed it to the user
				self:Print("Your version: "..version.." is outdated, newer version is: "..data[1])
				hasVerCheck = true;				
			end
		...
		elseif cmd == 'verTestReply' then
			-- data = class, guildRank, version
			if data then			
				if not data[1] or not data[2] or not data[3] then
					return -- we probably told them theirs is outdated and their old format sucks
				end
				if version < data[3] and not hasVerCheck then -- if we're outdated
					self:Print("Your version: "..version.." is outdated, newest version is: "..data[3])
					hasVerCheck = true;
				end
				tinsert(data, 2, sender) -- insert sender to the table
				RCLootCouncil_VersionFrame:AddPlayer(data) -- and add the data to the versionFrame							
			else -- if old version
				RCLootCouncil_VersionFrame:AddPlayer({nil, sender, "Unknown", data[3]})
			end
...
function RCLootCouncil_Mainframe.vote(id)
	...
	if id then
		...
		if entryTable[currentSession][id][11] then -- if unvote
			...
			if not mlDB.selfVote and GetUnitName("player", false) == entryTable[currentSession][id][1] then -- test that they may vote for themself
...
function RCLootCouncil_Mainframe.award(reason)
	...
	elseif isTesting then
		...
		for memberId = 1, 40 do
			local candidateName = GetMasterLootCandidate(memberId)
			if candidateName and candidateName == selection[1] then
				if #itemsToLootIndex > 0 and lootNum > 0 then -- if there really is something to loot
					...
					if lootQuantity > 0 then -- be certain there's an item
						GiveMasterLoot(itemsToLootIndex[lootNum], memberId); -- give the item
...
function RCLootCouncil_Mainframe.getML()
	...
	if lootMethod == 'master' and MLRaidID then
		...
		return name;
	elseif isRunning and (IsRealRaidLeader() == 1) then -- if masterlooting isn't on, turn it on, but only if we're running and are the raid leader	
		SetLootMethod("master", GetUnitName("player", false))
...
function RCLootCouncil_Mainframe.isCouncil()
	...
	if #currentCouncil > 0 then
		for _, v in ipairs(currentCouncil) do
			if v == GetUnitName("player", false) then
...
function RCLootCouncil:GetItemsFromMessage(msg, sender)
	...
	if isMasterLooter then
		...
		if theItem  then -- If they entered a valid item
			...
			self:debug("restOfMsg = "..restOfMsg)
			for k,v in ipairs(whisperKeys) do
				self:debug("whisperKeys["..k.."] = num = "..v.num.." key = "..v.key)
...
function RCLootCouncil:GetVariable(arg)
	...
	elseif arg == "council" then 
		if currentCouncil then return currentCouncil else return nil end
	end
end
...
function RCLootCouncil:AutoAward(index, awardTo, itemLink)
	...
	if GetUnitName("player", false) == awardTo then -- just take it
	...
	for memberId = 1, 40 do
		local canditateName = GetMasterLootCandidate(memberId)
		if canditateName and canditateName == awardTo then
			local _, item, lootQuantity = GetLootSlotInfo(index)
			if lootQuantity > 0 then -- be certain there's an item
				GiveMasterLoot(index, memberId); -- give the item
				self:debug(""..awardTo.." was Auto Awarded with "..item);
				self:Print(itemLink.." was Auto Awarded to "..awardTo..", Reason: "..db.otherAwardReasons[db.autoAwardReason].text)
			else
